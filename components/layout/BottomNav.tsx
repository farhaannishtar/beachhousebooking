...
export default function BottomNav() {
  ...
  return (
    <nav className=" h-[4.5rem] px-4 py-2 flex border-t-[1px] border-typo_light-100 items-center bottom-0 bg-white mt-8 sticky">
        ...
        <Link href="/protected/reports" className="flex flex-col flex-1 gap-2 justify-between items-center group">
        <span className={`${currentPath.includes('/protected/reports')?'!text-selectedButton':''} material-symbols-outlined text-typo_light-200 group-hover:!text-selectedButton`}>auto_graph</span>
        <h4 className={`${currentPath.includes('/protected/reports')?'!text-selectedButton':''} small-text text-link group-hover:!text-selectedButton`}>Reports</h4>
        </Link>

...
const handleGet = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  let propertiesInternal = (properties as string).split(',').map((property) => convertStringToProperty(property));
  ...
  for (let property of propertiesInternal) {
    let events = await listEvents(property, `${year}-${monthString}-01T00:00:00Z`, `${year}-${monthString}-31T23:59:59Z`)
    json.push(events.map((event) => {
      return {
